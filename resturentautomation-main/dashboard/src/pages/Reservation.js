...
const Reservation = () => {
  ...
  const [openAddWaste,setOpenAddWaste] = useState(false);
  const [selectedIngredients,setSelectedIngredients] = useState([])
  const [grandTotal,setGrandTotal] = useState(0.00)
  const [wastes, setWastes] = useState([]);
  const [paidAlready,setPaidAlready ] = useState(0.00)

  const handleCloseAddWaste= ()=>{
    setOpenAddWaste(false)
  }
  
  const handleAddWaste=(ref_no,supplier,ingredients_list, total_price, paid_price, due_price)=>{
    let newWastes = [...wastes,{ref_no: ref_no,supplier: supplier,ingredients_list: ingredients_list,total_price: total_price, paid_price: paid_price, due_price: due_price}]
    setWastes(newWastes)
    setOpenAddWaste(false)
  ...
  <>
    <Helmet>
      <title>Reservation | Client Portal</title>
    </Helmet>
    <Box
      sx={{
    ...
    >
      <Container maxWidth={false}>
        ...
        >
          ...
          <div>
          <Button type="text" style={{}} size="large" onClick={()=>{window.open(encodedUri)}}>
            Export
          </Button>
          <Button style={{margin:"20px"}} size="large" variant="contained" onClick={()=>{setRefno(wastes.length+1);setOpenAddWaste(true)}}>
            Add Reservation
          </Button>
          </div>

          <Dialog open={openAddWaste} onClose={handleCloseAddWaste}>
                <DialogTitle>Add Reservation</DialogTitle>
                <DialogContent>
                  <DialogContentText>
                  Adding Reservation manually will not trigger any notifications or payments. 
                ...
                <DialogActions>
                  <Button onClick={handleCloseAddWaste}>Cancel</Button>
                  <Button onClick={()=>handleAddWaste(wastes.length+1,supplierOption,selectedIngredients,grandTotal,paidAlready,(grandTotal-paidAlready))}>Add</Button>
                </DialogActions>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>TOTAL RESERVATION</TableCell>
            ...
            <TableBody>
              {wastes.length > 0 ? (
                wastes?.map((waste) => (
                  <TableRow hover key={waste.ref_no}>
                    <TableCell style={{  }}>
                      {waste.ref_no}
                    </TableCell>
                    
                    <TableCell>{waste.supplier}</TableCell>
                    <TableCell  align="right" >{waste.total_price}</TableCell>
                    <TableCell  align="right">{waste.total_price - waste.paid_price}</TableCell>
                  </TableRow>
                ))
              ) : (
