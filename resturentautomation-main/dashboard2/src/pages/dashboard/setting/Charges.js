...
const Charges = (props) => {
  ...
  useEffect(()=>{
   const  fetchdata = async () =>{
        const taxes  = await axios.get("http://localhost:5000/charges/");
        console.log(taxes)
        setrowstoadd(taxes)
    }
    fetchdata();
  ...
  const columns = [
    ...
    { label: "Description", minWidth: 100, maxWidth: 150 },
...
//   const rows = [createData("Tax", "MENU", "Order Type", "Value Type ", "value", "type", "Decription", "Active")];
  ...
  function submit(e){
    e.preventDefault();
    const fetchdata = async () => {
        const data = await axios.get('http://localhost:5000/charges/')
        console.log("sections:",data)
        setrowstoadd(data);
    };

    axios.post('http://localhost:5000/charges/add', {
        tax:newdata.tax, 
        menu: newdata.menu,
  ...
  const EditTable = (e, tabledata) => {
    e.preventDefault();
    // console.log(id);
    axios.post(`http://localhost:5000/charges/update/${tabledata._id}`, {
        tax:datainedit.tax, 
  ...
  const handleRemove = (e, id) => {
    e.preventDefault();
    axios.delete(`http://localhost:5000/charges/delete/${id}`).then(() => console.log('Charges deleted.'))
              ...
              {rows && rows?.data.map((row, id) => {
                return (
                  <TableRow key={id}>
                    ...
                    <TableCell align="center">{row.menu && (row.menu.length !== 1  ? row.menu.map((item)=>`${item}, `): row.menu)}</TableCell>
                    <TableCell align="center">{row.order_type && (row.order_type.length !== 1  ? row.order_type.map((item)=>`${item}, `): row.order_type )}</TableCell>
