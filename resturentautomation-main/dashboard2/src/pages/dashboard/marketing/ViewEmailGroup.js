...
const ViewEmailGroup = (props) => {
  const classes = useStyles();
  const navigate = useNavigate();

  const columns = [
    { label: "Sr. No.", minWidth: 40, maxWidth: 60 },
    { label: "Email", minWidth: 100, maxWidth: 200 },
    { label: "Actions", minWidth: 20, maxWidth: 50 }
  ];

  const createData = (email) => {
    return { email };
  };

  const rows = [createData("email@gmail.com")];
