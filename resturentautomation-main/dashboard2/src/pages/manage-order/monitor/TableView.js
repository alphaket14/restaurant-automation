...
const TableView = () => {
  ...
  const [sectionCount, setSectionCount] = useState([])//Section wise table count 
  useEffect(() => {
    const fetchdata = async () => {
      ...
      const booked = await axios.get('https://vast-pink-meerkat-suit.cyclic.app/table/booked');
      const payement_due = await axios.get('https://vast-pink-meerkat-suit.cyclic.app/table/paymentdue');
      const sectionwise_count = await axios.get('https://vast-pink-meerkat-suit.cyclic.app/table/count');
      setSectionCount(sectionwise_count);
      setTableData(data);
      setSectionData(section_data);
      console.log("tables:",data);
      console.log("sections:",section_data);
      console.log("Section wise count : ", sectionCount.data);
      ...
      console.log("this are tableData"+tableData);
      // console.log("this is satatus" + tableData?.data.status)


      const table_data = tableData && tableData?.data.filter((item)=>{
        return item.status === "Vacant"
      })
      const vacantCount = table_data.length;
      
      
      
      console.log(table_data);
      setTables(table_data);
      console.log("This is Booked Data ===>" + booked.data.length + "  " + tables)
      console.log("This is Vacant Data ===>" + table_data.length)
      setVacantNumber(vac_table.data.length);
      setBookedNumber(booked.data.length);
  ...
  // Add new Table Data
    ...
    const onSectionChangeHandler = (event, newValue) => {
      const table_data = tableData?.data.filter((item)=>{
        return item.section === newValue  && item.status === "Vacant"
      })
      console.log(table_data)
      setTables(table_data)
          ...
          <Grid container direction="column" justifyContent="space-between">
            ...
            <Grid item sx={{ mt: 1 }}>
              <Grid item container direction="column" rowSpacing={2}>
                ...
                <Grid fullWidth item>
                  <Button fullWidth  color="secondary"  component={NavLink}
                    variant="outlined"
                    to="/manageorder/booked-table">
          ...
          <Grid item container direction="column">
            <Grid item container sx={{ padding: 1 }} spacing={2}>
              ...
              tables && tables?.map((item)=>(
                  ...
                  <Grid item xs={2} key={item._id} sx={{width:"100%", margin:"0px"}}>
                  <Paper
                    onClick={()=>navigate("/manageorder/pos-invoice",{state: {item:item, new: true}})}
                    fullWidth
                    elevation={6}
                    sx={{
                      display: "flex",
